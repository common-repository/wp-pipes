	...
	public static function show_message($bootstrap = false)
	{
		if (isset($_SESSION['PIPES']['messages']) && count($_SESSION['PIPES']['messages'])) {
			...
			foreach ($msgs as $msg) {
				...
				// Check type of class to display msg
				if ($msg['type'] === 'error') {
					$msgContent = '<div id="message" class="error"><p>' . $msg['msg'] . '</p></div>';
				} elseif ($bootstrap) {
					$msgContent = '<div class="alert alert-' . $msg_class . ' fade in">
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
								<div>' . $msg['msg'] . '</div>
							</div>';
				} else {
					$msgContent = '<div id="message" class="updated"><p>' . $msg['msg'] . '</p></div>';
				}
